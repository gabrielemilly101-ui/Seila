-- ============================================================
--             MATRIX HUB LIBRARY v4.0 (COM GUIL)
-- By @EmillyGabriel
-- ============================================================
...
-- ============================================================
local menuWidth  = math.min(620, Viewport.X - 20)
local menuHeight = math.min(380, Viewport.Y - 80)
...
--                      SIDEBAR
-- ============================================================
local sideWidth = math.min(150, menuWidth * 0.24)
...
function Library:MakeWindow() return Menu end
function Library:MinimizeButton() return MinBtn end
...
function Library:AddToggle(parent, name, default, callback)
    ...
    local tBg = Create("Frame", frame, {
        ...
        BackgroundColor3 = state.Value and Color3.fromRGB(0, 100, 0) or Color3.fromRGB(20, 20, 20),
        ZIndex           = 7
    })
    ...
    local dot = Create("Frame", tBg, {
        ...
        Position         = state.Value and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7),
        BackgroundColor3 = Color3.fromRGB(0, 255, 0),
        ZIndex           = 8
    })
...
-- ============================================================
--   AddCodeBox com NUMERAÇÃO DE LINHAS (a grande novidade)
-- ============================================================
function Library:AddCodeBox(parent, label, placeholder)
    local frame = Create("Frame", parent, {
    ...
    local gutter = Create("Frame", frame, {
        Size             = UDim2.fromOffset(35, 0),
        Position         = UDim2.new(0, 4, 0, 26),
        BackgroundColor3 = Color3.fromRGB(0, 4, 0),
        BorderSizePixel  = 0,
        ZIndex           = 7,
        ClipsDescendants = true
    })

    local gutterScroll = Create("ScrollingFrame", gutter, {
        Size                   = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        CanvasSize             = UDim2.new(),
        AutomaticCanvasSize    = Enum.AutomaticSize.Y,
    ...
    Create("UIListLayout", gutterScroll, {
    ...
    local codeBox = Create("TextBox", scrollBox, {
        Size                   = UDim2.new(1, 0, 1, 0),
        MinimumSize            = Vector2.new(200, 150),
    ...
    local function syncGutter()
        gutterScroll.CanvasPosition = Vector2.new(0, scrollBox.CanvasPosition.Y)
    end
    scrollBox:GetPropertyChangedSignal("CanvasPosition"):Connect(syncGutter)

    -- Atualiza a numeração de linhas
    local function updateGutter()
        gutterScroll:ClearAllChildren()
        Create("UIListLayout", gutterScroll, {
            Padding   = UDim.new(0, 2),
            SortOrder = Enum.SortOrder.LayoutOrder
        })
        ...
        for i = 1, math.max(lineCount, 1) do
            local ln = Create("TextLabel", gutterScroll, {
    ...
    codeBox:GetPropertyChangedSignal("Text"):Connect(updateGutter)
    updateGutter()
...
-- ============================================================
--   AddOutputBox: caixa de leitura (output do Guil)
-- ============================================================
function Library:AddOutputBox(parent, label)
    local frame = Create("Frame", parent, {
    ...
    Corner(frame, UDim.new(0, 4))
    Stroke(frame, Color3.fromRGB(0, 100, 255), 1)
    ...
    local scroll = Create("ScrollingFrame", frame, {
        ...
        AutomaticCanvasSize  = Enum.AutomaticSize.Y,
        ScrollBarThickness   = 3,
        ScrollBarImageColor3 = Color3.fromRGB(0, 100, 255),
        ZIndex               = 7
    })
    Corner(scroll, UDim.new(0, 4))

    local out = Create("TextLabel", scroll, {
    ...
    frame.SetText = function(t) out.Text = t end
    frame.GetText = function() return out.Text end
    frame.Append  = function(t) out.Text = out.Text .. "\n" .. t end
...
function Library:AddDropdown(parent, name, options, default, callback)
    ...
    local function toggleList()
        ...
        local targetH = list.Visible and (30 + math.min(#options, 4) * 25) or 30
...
function Library:AddParagraph(parent, title, text)
    ...
    Create("UIListLayout", frame, {Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder})
